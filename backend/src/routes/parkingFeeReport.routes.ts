...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const report = await prisma.parkingFeeReport.create({
      data: {
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const reports = await prisma.parkingFeeReport.findMany({
      where,
      include,
      orderBy: { date: "desc" },
    });
...
router.put(
  ...
  requireAdminManagerOrRegionManager,
  asyncHandler(async (req, res) => {
    const report = await prisma.parkingFeeReport.findUnique({ where: { id: req.params.id } });
    if (!report) return res.status(404).json({ error: "找不到此停車費回報" });

    if (req.user!.role === "REGION_MANAGER") {
      const managedIds = await getManagedUserIds(req.user!.id);
      if (!managedIds.includes(report.employeeId)) {
        return res.status(403).json({ error: "您只能審核自己區域成員的停車費回報" });
      }
    }
    ...
    const updated = await prisma.parkingFeeReport.update({
...
router.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (role === "ADMIN" || role === "MANAGER") {
      ...
      const managedIds = await getManagedUserIds(req.user!.id);
      if (!managedIds.includes(report.employeeId)) {
        return res.status(403).json({ error: "您只能刪除自己區域成員的停車費回報" });
      }
    } else {
      // 一般員工只能撤回自己的 PENDING
      if (report.employeeId !== req.user!.id) {
        return res.status(403).json({ error: "僅能刪除自己的停車費回報" });
      }
      if (report.status !== "PENDING") {
        return res.status(400).json({ error: "僅能撤回待審核的停車費回報" });
      }
    }

    await prisma.parkingFeeReport.delete({ where: { id: req.params.id } });
    res.status(204).end();
  })
);
