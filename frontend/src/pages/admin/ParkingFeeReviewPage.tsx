...
import type { ParkingFeeReport, User, VehicleType } from "../../api/types";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-sm rounded-lg bg-white p-5 shadow-lg">
        <h3 className="mb-3 text-base font-semibold text-gray-800">駁回停車費回報</h3>
...
interface VehicleGroup {
  ...
  plateNumber: string;
  total: number;
  reports: ParkingFeeReport[];
}

function buildVehicleGroups(reports: ParkingFeeReport[], type: VehicleType): VehicleGroup[] {
...
function VehicleStatsTab({
  ...
  useEffect(() => {
    ...
    setError(null);
    apiClient
      .get<ParkingFeeReport[]>("/parking-fee-reports", { params: { year, month, status: "APPROVED" } })
...
export function ParkingFeeReviewPage() {
  ...
  async function loadReports() {
    ...
    try {
      ...
      const { data } = await apiClient.get<ParkingFeeReport[]>("/parking-fee-reports", { params });
      setReports(data);
    } catch (err) {
      setError(getErrorMessage(err));
    } finally {
  ...
  async function handleApprove(id: string) {
    setApprovingId(id);
    try {
      await apiClient.put(`/parking-fee-reports/${id}/approve`);
      await loadReports();
    } catch (err) {
      setError(getErrorMessage(err));
    } finally {
  ...
  async function handleReject(id: string, reason: string) {
    await apiClient.put(`/parking-fee-reports/${id}/reject`, { rejectReason: reason });
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-2">
        ...
        <h1 className="text-xl font-semibold text-gray-800">停車費審核</h1>
      </div>
          ...
          <div className="rounded-lg border border-gray-200 bg-white shadow-sm">
            {loading ? (
              ...
              <p className="p-4 text-sm text-gray-400">
                {tab === "pending" ? "目前沒有待審核的停車費回報" : "查無紀錄"}
              </p>
            ) : (
