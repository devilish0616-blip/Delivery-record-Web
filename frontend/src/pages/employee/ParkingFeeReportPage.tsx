...
import { CheckCircle, Clock, ParkingSquare, Trash2, XCircle } from "lucide-react";
import { apiClient, getErrorMessage } from "../../api/client";
import type { ParkingFeeReport, VehicleType } from "../../api/types";
...
export function ParkingFeeReportPage() {
  const now = new Date();
  const [year, setYear] = useState(now.getFullYear());
  const [month, setMonth] = useState(now.getMonth() + 1);
  const [reports, setReports] = useState<ParkingFeeReport[]>([]);
  ...
  async function loadReports() {
    ...
    setError(null);
    try {
      const { data } = await apiClient.get<ParkingFeeReport[]>("/parking-fee-reports/my", {
        params: { year, month },
      });
      setReports(data);
    } catch (err) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!formDate) { setFormError("請選擇停車日期"); return; }
    if (isNaN(amt) || amt <= 0) { setFormError("請輸入有效的停車費金額"); return; }

    setSubmitting(true);
    try {
      await apiClient.post("/parking-fee-reports", {
        date: formDate,
        amount: amt,
  ...
  async function handleDelete(id: string) {
    setDeleting(true);
    try {
      await apiClient.delete(`/parking-fee-reports/${id}`);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2">
        ...
        <h1 className="text-xl font-semibold text-gray-800">停車費回報</h1>
      </div>

      {/* 新增表單 */}
      <div className="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
        <h2 className="mb-4 text-sm font-semibold text-gray-700">新增停車費回報</h2>
        <form onSubmit={handleSubmit} className="space-y-3">
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-5">
            <div>
              <label className="mb-1 block text-sm font-medium text-gray-700">停車日期</label>
              <input
                type="date"
                value={formDate}
            ...
            <div>
              <label className="mb-1 block text-sm font-medium text-gray-700">車輛類型</label>
      ...
      <div>
        ...
        <div className="rounded-lg border border-gray-200 bg-white shadow-sm">
          {loading ? (
            ...
            <p className="p-4 text-sm text-gray-400">本月尚無停車費回報紀錄</p>
          ) : (
